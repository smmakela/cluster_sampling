...
parameters {
  ...
  vector[K] eta1;
  vector<lower=0>[S] mu;
  vector<lower=0>[S] recip_sqrt_alpha_nb;
}
transformed parameters {
  ...
  beta0 = alpha0 + gamma0 * log_Nj_sample + stratum_matrix * Kappa0 + eta0 * sigma_beta0;
  beta1 = alpha1 + gamma1 * log_Nj_sample + stratum_matrix * Kappa1 + eta1 * sigma_beta1;

...
generated quantities {
  ...
  //real Tx_star; // total size based on
  vector[J_pop] Mj_all;
  //int kk;
  real Mj_tot_est;
  ...
  logMj_star <- log(Mj_all) - mean(log(Mj_all));
  ...
  for (j in 1:J_pop) {
    ...
    if (j <= J_sam) {
      ...
      //kk <- kk + 1;
      beta0j_draw <- gamma0 + (gamma1 * logMj_star[j]) + (epsilon1 * sigma_beta0);
      beta1j_draw <- alpha0 + (alpha1 * logMj_star[j]) + (epsilon2 * sigma_beta1);

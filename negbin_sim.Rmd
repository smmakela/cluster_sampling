...
Here's the distribution of posterior p-values:
```{r, echo = FALSE}
# dplyr::summarise(group_by(p.vals.df, stat),
#                  mean_pval = mean(p_value))
# ggplot(p.vals.df, aes(x = p_value)) +
#   geom_line(stat = "density") +
#   facet_wrap(~ stat) +
#   theme_bw()
```

What if the cluster sizes had instead come from a lognormal distribution? What would things look like then?

```{r}
Nj.pop <- exp(rnorm(n = J)) # exp(N(0, 1))
stan.model <- stan_model(file = "src/lognormal.stan")
model.name <- "lognormal"
n.iter <- 1000
n.chains <- 4
n.sims <- 1
sim.res <- negbin_sim(Nj.pop, J, K, model.name, stan.model, n.iter, n.chains, n.sims)
```
better prior for sigma? esp in real application
flaw in continuous model -- in discrete data, you have another piece of info: no clusters are 0, lower bound on cluster size
still lower bound in continuous : truncated lognormal?
cauchy prior on sigma is too weak?